...
while ($mentee_row = $res->fetch_assoc()) {
    ...
    // Retrieve mentors with matching domains
    $query = "SELECT * FROM mentors WHERE domain = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('s', $domain);
    $stmt->execute();
    $mentor_res = $stmt->get_result();

    // Check if mentors with matching domains are found
    if ($mentor_res->num_rows > 0) {
        // Iterate through mentors with matching domains
        while ($mentor_row = $mentor_res->fetch_assoc()) {
            // Check if the mentor has available slots for more students
            if ($mentor_row['assignedstudents'] < $max_students_per_mentor) {
                // Assign the mentor to the mentee
                $mentor_id = $mentor_row['id'];
                $mentee_id = $mentee_row['id']; // Assuming you have an 'id' column in your userregistration table
                ...
                // Exit the loop after assigning the mentor
                break;
            }
        }
    }
